...
const deleteBank = async (req, res) => {
    try {
        const bankId = await req.params.bankId
        const bank = await Bank.findOneAndDelete({ _id: bankId })
        console.log(bank);
        if (!bank) {
            return res.status(500).json("internal server error")
        }
        res.status(204).json("bank deleted successfully")
    } catch (error) {
        console.log(error.message);
    }
}

module.exports = { addNewBank, getUserBanks, addBalance, withdrawBalance , deleteBank }
